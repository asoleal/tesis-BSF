...
\end{tikzpicture}
\caption{Esquema del balance conservativo de la Definición~\ref{def12}. La cantidad $x(t)$ dentro del volumen de control cambia por dos mecanismos:
...
este documento. El modelo de Eriksen \citep{eriksenDynamicModellingFeed2022} es una red de balances
...
y la carga inicial del lote como condición inicial.
\end{remark}
...
\end{tikzpicture}
\caption{Interpretación del supuesto de mezcla homogénea (Definición~\ref{def13}).
...
externos: masa de sustrato, y moles de CO$_2$, CH$_4$, O$_2$ y vapor de agua
en el volumen de aire del contenedor.
\end{remark}
...
\draw[interno] (L) to[bend left=25] node[above right=-1pt, pos=0.55]
  {\footnotesize reciclaje ($r_L < 0$)} (A);

% Tres fuentes de CO2 (rótulos alejados de los nodos)
\draw[resp] (B.south) -- ++(0,-1.0)
  node[below]{$r_{\mathrm{CO_2},m} = mB$};
\draw[resp] (-1.5,1.5) -- ++(-1.1,1.1)
...
\end{tikzpicture}
\caption{El modelo de \citet{eriksenDynamicModellingFeed2022} como red de compartimentos (Definici\'on~\ref{def14}). El sustrato es un forzamiento externo que alimenta el compartimento de asimilados $A$ a tasa $r_A = aB$; las flechas internas representan s\'intesis de biomasa estructural ($r_B$), acumulaci\'on de l\'ipidos ($r_L$) y reciclaje de reservas hacia el pool de asimilados cuando $r_L < 0$. Las tres flechas rojas son las fuentes de producci\'on de CO$_2$: mantenimiento (proporcional a $B$), costo de s\'intesis estructural y costo de s\'intesis de l\'ipidos, cuya suma es la producci\'on total (Ec.~\eqref{eq:rco2total}).}
\label{fig:red-eriksen}
\end{figure}

En el modelo de Eriksen, la producción total de CO$_2$ se descompone en tres
procesos metabólicos distintos \citep{eriksenDynamicModellingFeed2022}. El primero es el
...
El modelo de \citet{eriksenDynamicModellingFeed2022} se obtiene al particularizar la
Definición~\ref{def11} con el vector de estado, las entradas y las mediciones
\begin{equation}
...
mediciones de humedad relativa, CH$_4$ y O$_2$. En particular, la medición
simultánea de CO$_2$ y O$_2$ habilita el \emph{cociente respiratorio}
\begin{equation}
RQ \;=\; \frac{r_{\mathrm{CO_2}}}{r_{\mathrm{O_2}}},
\label{eq:rq}
\end{equation}
donde $r_{\mathrm{O_2}} > 0$ denota la tasa de \emph{consumo} de oxígeno
---símbolo propio de este documento, pues el modelo de Eriksen no incluye el
oxígeno---, como verificación independiente del metabolismo: valores de
$RQ$ cercanos a $1.0$ indican oxidación de carbohidratos, cercanos a $0.7$,
oxidación de lípidos~\citep{lightonMeasuringMetabolicRates2008}. Lo anterior
 anterior plantea el problema de estimación de
estados y parámetros que abordaremos con redes neuronales informadas por
físicafísica~\citep{raissiPhysicsinformedNeuralNetworks2019a}. En síntesis, esta sección ha establecido el lenguaje formal sobre el
que se construye el resto del documento: un sistema dinámico
...
\citet{eriksenDynamicModellingFeed2022} emerge así como un caso particular de esta
