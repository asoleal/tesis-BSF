...
\begin{remark}[Identificación en lazo cerrado]\label{rem:id-lazo}
En \eqref{eq:lazo-cerrado}, las entradas $\mathbf{u}$ están correlacionadas
con las perturbaciones por construcción: el controlador reacciona a ellas.
Es la dificultad clásica de la identificación en lazo cerrado: un estimador
entrenado solo con datos de operación regulada puede aprender la ley
$\pi$ en lugar de la fisiología. En consecuencia, los datos de los cierres
de ventilación (Remark~\ref{rem:cierres}) y de las ventanas con consignas
en escalón desempeñarán el rol de datos de \emph{identificación}, mientras
que la operación regulada normal se reserva para validación
(cf.~Sección~6).
\end{remark}

\subsection{Conmutación de la ventilación}

\begin{definition}[Entrada de ventilación conmutada]\label{def42}
Sea $\{t_k^{c}\}_{k \geq 1}$ el calendario de cierres programados
(Remark~\ref{rem:cierres}) y $\Delta$ la duración de cada cierre. La
ventilación opera en dos \emph{modos}:
\begin{equation}
Q(t) = \begin{cases}
Q_{\mathrm{op}}(t), & \text{modo ventilado: }
t \notin \bigcup_k [t_k^{c},\, t_k^{c} + \Delta],\\[4pt]
0, & \text{modo cerrado: }
t \in [t_k^{c},\, t_k^{c} + \Delta],\; k = 1, 2, \dots
\end{cases}
\label{eq:q-conmutada}
\end{equation}
donde en modo ventilado $Q_{\mathrm{op}}(t)$ está determinado por la ley de
control \eqref{eq:ley-control}, y en modo cerrado la ventilación se anula
por decisión del protocolo, suspendiéndose temporalmente la regulación.
\end{definition}
...
Ec.~\eqref{eq:modo-cerrado} es la base del método de cámara estática: las
...
metabólico limitante, $r_A = aB$ \citep{eriksen2022}, con $a$ modulada por
...
(Ec.~1 de \citet{eriksen2022}), de modo que $\dot{A} \approx 0$ en
...
\citet{eriksen2022} fue formulado y ajustado con parámetros constantes a
temperatura de cría controlada, con buen desempeño frente a datos de
distintos sustratos y humedades \citep{eriksen2022,eriksen2024}. Dado que
