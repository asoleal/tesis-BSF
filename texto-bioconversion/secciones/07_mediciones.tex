...
\begin{description}
  ...
  $H$ (SHT35), CO$_2$ (NDIR) y O$_2$ (celda galvánica) en la corriente de salida, más el CH$_4$ del sensor MOS como señal de supervisión; la cuantificación de CH$_4$ es por cromatografía en los cierres (Sección~\ref{sec:caracterizacion}):
  \[
...
\begin{remark}[El O$_2$ como supervisión, no como tasa]\label{rem:o2-supervision}
La celda galvánica KE-25 tiene resolución $\sim 0{,}1\%$, mientras que la
variación de O$_2$ durante un cierre es del orden de $0{,}01\%$: la
pendiente de O$_2$ no es medible en línea. El sensor cumple entonces una
función de supervisión ---verificar el piso de seguridad de la
Ec.~\eqref{eq:delta-max}--- y la tasa de consumo $R_{\mathrm{O_2}}$ se
obtiene del modelo con $RQ$ fijo (S6) o de los muestreos GC de cada cierre,
que sí resuelven la diferencia.
\end{remark}
...
\begin{description}
  ...
  Observable directamente: $T$, $w$, $c_{\mathrm{CO_2}}$ y $c_{\mathrm{O_2}}$ medidos por identidad en línea, con $w$
  obtenido de $H$ y $T$ vía psicrometría; $c_{\mathrm{CH_4}}$ queda observada de
  forma discreta en los muestreos GC de cada cierre.
  ...
  combinan $mB$, $Y_B r_B$ y $Y_L r_L$. Los muestreos GC
  aportan además el cociente respiratorio~\eqref{eq:rq-medido} en los
  instantes de cierre, que discrimina el sustrato metabólico oxidado (el
  sensor de O$_2$ en línea no resuelve la pendiente,
  Observación~\ref{rem:o2-supervision}).
\end{description}
...
\[
  ...
  \qquad i \in \{\mathrm{CO_2}, \mathrm{CH_4}\},
\]
independientes de $Q$ y sin modelado fisiológico: la pendiente de
CO$_2$ se ajusta en línea, la de CH$_4$ sobre los tres puntos GC del
cierre, y la de O$_2$ se obtiene vía $RQ$ (Observación~\ref{rem:o2-supervision}). La ventana $\Delta$ se
...
\begin{enumerate}
  ...
  el $RQ$ de cada cierre (vía GC) y la razón CH$_4$/CO$_2$ de la alarma
  MOS, con la firma de terminación de la Observación~\ref{rem:tf} evaluada
  en línea.
