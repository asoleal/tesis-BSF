...
\begin{itemize}
    \item \textbf{Datos Experimentales:} Series temporales de respiración (tasas netas de $CO_2$) obtenidas tras un riguroso proceso de extracción, filtrado de saturación y corrección de línea base[cite: 5, 20, 23].
    \item \textbf{Restricciones Físicas:} El cumplimiento de una ecuación diferencial ordinaria (EDO) que descompone la respiración en costos de crecimiento, síntesis de lípidos y mantenimiento celular[cite: 24, 25].
\end{itemize}

Mediante esta metodología, se busca no solo reconstruir las trayectorias de crecimiento, sino también descubrir los parámetros cinéticos fundamentales ---tales como el coeficiente de mantenimiento ($m$) y los rendimientos de conversión ($Y_g, Y_l$)--- que definen la eficiencia metabólica de \textit{H. illucens} frente a variaciones en la calidad del sustrato[cite: 26, 30].
