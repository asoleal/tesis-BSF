...
\scriptsize
\begin{itemize}
    \item El entorno agresivo (HR $>80\%$, amoniaco, volátiles) \textbf{degrada} sensores y acelera \textit{e-waste}
    \item Mitigación: diseño modular (sensor reemplazable, ESP32 reutilizable) y microcontroladores de bajo consumo \parencite{dharCarbonImpactArtificial2020}
\end{itemize}
\end{alertblock}

\vspace{0.12cm}

\begin{block}{Fronteras del modelo}
\scriptsize
\begin{itemize}
    \item \textbf{Especificidad:} generalización limitada a sustratos del entrenamiento
    \item \textbf{Resolución 0D:} ignora gradientes internos de temperatura y oxígeno
    \item \textbf{Régimen:} validado en batch; sistemas continuos requerirían recalibración
\end{itemize}
\end{block}
...
\scriptsize
\begin{itemize}
    \item \textbf{Sobreajuste:} el prototipo puede ajustarse excesivamente al laboratorio
    \item \textbf{Fragilidad:} pérdida de robustez ante sustratos y microclimas reales
    \item \textbf{Opacidad:} IA sin anclaje físico genera predicciones biológicamente inviables
\end{itemize}
\end{alertblock}
...
\scriptsize
\begin{itemize}
    \item El DEB ancla la IA: impone restricciones biológicas en la función de pérdida
    \item Penaliza predicciones imposibles (ej.\ crecimiento negativo, CO$_2$ $>$ carbono ingerido)
    \item Delimita el \textbf{dominio de validez} y la necesidad de recalibración
\end{itemize}
\end{block}
...
\begin{alertblock}{Brecha digital rural}
\scriptsize
\begin{itemize}
    \item \textbf{Zonas grises:} conectividad nula excluye arquitecturas centralizadas en nube
    \item \textbf{Barrera:} dependencia de soporte técnico especializado limita la adopción
\end{itemize}
\end{alertblock}
...
\scriptsize
\begin{itemize}
    \item Procesamiento crítico directamente en el nodo ESP32
    \item Operación autónoma y sincronización asíncrona de datos
    \item Costo marginal para pequeños productores agroindustriales
\end{itemize}
\end{block}
...
\scriptsize
\begin{itemize}
    \item Alertas ante fugas o cambios térmicos súbitos
    \item Intervención manual inmediata
    \item Cumplimiento fitosanitario (ICA/UE)
\end{itemize}
\end{block}
...
\scriptsize
\begin{itemize}
    \item \textit{Timestamping} digital de registros de emisiones
    \item Datos auditables bajo estándares IPCC y Verra
    \item Contrasta \textit{greenwashing} y habilita certificación de créditos de carbono
\end{itemize}
\end{alertblock}
...
\begin{block}{\scriptsize 1. Transformación técnica}
\tiny
\begin{itemize}
...
\begin{block}{\scriptsize 2. Transformación ambiental}
\tiny
\begin{itemize}
    \item De factores de emisión estáticos e intransferibles a \textbf{Inventario Dinámico de Carbono}
...
\begin{block}{\scriptsize 3. Transformación operativa}
\tiny
\begin{itemize}
    \item De gestión empírica a decisiones basadas en datos
    \item Alertas tempranas de anaerobiosis (semáforo verde/ámbar/rojo)
    \item Edge computing para productores agroindustriales del Sur Global sin dependencia de la nube
\end{itemize}
\end{block}
