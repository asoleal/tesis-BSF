...
\begin{table}[htbp]
    ...
    \begin{tabular}{@{}clcc@{}}
        ...
        $B(t)$ & Biomasa estructural & g C & $[B_{\min}, B_{\max}]$ \\
        $L(t)$ & Lípidos de reserva & g C & $[0, L_{\max}]$ \\
        $C_{\ch{CO2}}(t)$ & Carbono emitido como \ch{CO2} & g C & $[0, C_{\mathrm{tot}}]$ \\
        \bottomrule
    \end{tabular}
\end{table}
...
La variable $C_{\ch{CO2}}(t)$ acumula el carbono emitido como dióxido de carbono. Su derivada temporal $\dot{C}_{\ch{CO2}}(t)$ se relaciona directamente con el flujo másico medido por el sensor NDIR mediante la ecuación \eqref{eq:cap3_puente_sensor}, permitiendo la confrontación modelo--sensor en el Capítulo \ref{chap:hibrido}.
...
\end{equation}
donde $B_{\min} > 0$ representa la biomasa estructural mínima compatible with la viabilidad celular (aproximadamente la biomasa inicial $B_0$). La restricción $B \geq B_{\min}$ evita la extinción biológica del sistema, aunque en la práctica experimental la mortalidad larval es despreciable (inferior al $5\%$) en condiciones óptimas \parencite{salamEffectDifferentEnvironmental2022}.
...
\end{align}
donde $M_{\mathrm{sustrato}}$ es la masa húmeda inicial de sustrato (\SI{14}{\kilo\gram} en el reactor de \SI{20}{\litre}), $w_{\ch{C}}$ la fracción de carbono en base húmeda del sustrato ($0.35$--$0.45$ para pulpa de café y cáscaras de frutas), $f_{\mathrm{inerte}}$ la fracción lignocelulósica ($0.10$--$0.15$), $m_{\mathrm{larvas}}$ la biomasa húmeda inicial por larva (\SI{0,005}{\gram} en estadio L3 sincronizado), y $N_{\mathrm{larvas}}$ el número total de larvas sembradas (\num{5000} larvas/reactor para una densidad de \SI{2,5}{\gram\per\litre}).
