...
Si bien los modelos mecanicistas actuales proporcionan una base teórica indispensable, su aplicación directa en entornos operativos dinámicos enfrenta limitaciones intrínsecas. La literatura \parencite{bekkerImpactSubstrateMoisture2021a,rossiEstimatingDynamicsGreenhouse2024a} indica que estas formulaciones, validadas principalmente con dietas estandarizadas bajo condiciones controladas, a menudo simplifican la complejidad fisicoquímica de las mezclas de residuos reales. En escenarios de campo, donde la heterogeneidad del sustrato es la norma, el uso de parámetros cinéticos fijos restringe la capacidad del modelo para predecir con exactitud. Esta brecha operativa subraya la necesidad de enfoques híbridos, donde el aprendizaje automático capture la variabilidad no modelada —atribuible a la incertidumbre nutricional de la mezcla— y ajuste dinámicamente las proyecciones del modelo base a las condiciones cambiantes del proceso.
...
Sin embargo, esta integración enfrenta desafíos prácticos que definen la arquitectura del sistema: (i) la dependencia de datos masivos en los algoritmos de AAA puros, lo que genera riesgo de \textit{overfitting} y problemas de «arranque en frío» (\textit{cold start}) en instalaciones nuevas. Esta limitación se mitiga aquí utilizando el modelo mecanístico como un \textit{prior} informativo robusto, garantizando funcionalidad desde el primer día sin necesidad de históricos extensos; (ii) la validación cruzada y el ajuste de hiperparámetros, que requieren estrategias específicas para contextos de \textit{Small Data}; (iii) el costo computacional de arquitecturas complejas (redes neuronales) frente a opciones más ligeras. Por ello, esta tesis adopta un enfoque escalonado: fundamentar la operación en el modelo mecanístico, refinar las predicciones mediante modelos interpretables (regresión) a medida que se generan datos, y evaluar redes neuronales multicapa solo cuando la base de datos acumulada garantice una mejora significativa sobre los métodos más simples.
...
Esta investigación se alinea con los principios de \textit{soberanía de datos} y \textit{diseño frugal}. La arquitectura propuesta es descentralizada y computacionalmente eficiente: el procesamiento crítico ocurre en el borde (\textit{edge computing}), priorizando algoritmos optimizados que pueden ejecutarse en hardware de bajo costo sin depender de infraestructuras en la nube centralizadas y costosas. Esto no solo reduce la latencia y los costos operativos, sino que garantiza que los datos sensibles permanezcan bajo el control local del productor, operando incluso en condiciones de conectividad intermitente. Filosóficamente, el monitoreo ambiental no debe ser una herramienta de vigilancia, sino un instrumento de empoderamiento que permita a los productores agroindustriales validar sus prácticas ante mercados globales, sin alienar su conocimiento local.
