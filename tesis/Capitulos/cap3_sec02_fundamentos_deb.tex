...
\begin{description}
    \item[Ingesta ($\dot{I}$)] Velocidad a la que el carbono del sustrato pasa al tracto digestivo. Está limitada por la densidad larval, la humedad del sustrato y la disponibilidad física de alimento. \textcite{dienerConversionOrganicMaterial2009} reportan tasas máximas de 100 mg alimento/larva/día en condiciones óptimas.
    ...
    \item[Síntesis de biomasa ($\dot{G}_B$) y de lípidos ($\dot{G}_L$)] Flujos de crecimiento que translocan carbono desde la reserva $E$ hacia los compartimentos estructural y lipídico. La prioridad entre $\dot{G}_B$ y $\dot{G}_L$ cambia con el estadio larval: dominan la síntesis de estructura en L3--L4, mientras que la lipogénesis se acelera en L5 previo a la metamorfosis \parencite{eriksenDynamicModellingFeed2022,grausaMetabolicModelingHermetia2023}.

    \item[Disipación ($\dot{D}$)] Carbono perdido irrevocablemente como \ch{CO2} durante los procesos de mantenimiento y síntesis. En el marco DEB, la respiración total es la suma de dos términos disipativos:
    \begin{equation}
        \dot{D}(t) = \dot{M}(t) + \bigl(1-\kappa_G\bigr)\dot{G}_B(t) + \bigl(1-\kappa_L\bigr)\dot{G}_L(t),
...
La producción de \ch{CO2} medida por los sensores NDIR —expresada como flujo másico $\dot{m}_{\ch{CO2}}(t)$ en g/min— se identifica con la disipación $\dot{D}(t)$ escalada al reactor completo, más una contribución microbiana $\dot{D}_{\mu}(t)$ que modela la respiración heterotrófica del sustrato no ingerido:
\begin{equation}
    \dot{m}_{\ch{CO2}}(t) = \frac{M_{\ch{CO2}}}{M_{\ch{C}}}\,\Bigl[\,N_{\mathrm{larvas}}\,\dot{D}(t) + \dot{D}_{\mu}(t)\Bigr],
    \label{eq:cap3_puente_sensor}
\end{equation}
donde $M_{\ch{CO2}} = 44.01$ g/mol y $M_{\ch{C}} = 12.01$ g/mol son las masas molares del dióxido de carbono y del carbono elemental, respectivamente, y $N_{\mathrm{larvas}}$ es el número de larvas en el reactor, derivado de la biomasa inicial y la curva de supervivencia. La ecuación \eqref{eq:cap3_puente_sensor} constituye el puente cuantitativo entre la teoría bioenergética y el dato crudo del Capítulo \ref{chap:instrumentacion}: permite interpretar una pendiente de concentración de 200 ppm/30 min no como un número aislado, sino como la manifestación física de los costos metabólicos de un conjunto larval en un estado fisiológico determinado.
