...
\end{equation}
donde $\tilde{b}(t) = B(t)/N_{\mathrm{larvas}}$ es la biomasa estructural por larva (\si{\gram C \per larva}) y $\tilde{\imath}$ es la tasa de ingesta específica (\si{\gram C \per larva \per \minute}), función creciente de la biomasa individual ---que determina la superficie bucal y la capacidad de procesamiento--- y de la disponibilidad de sustrato $S$. La dependencia térmica se incorpora mediante el factor $f(T)$ definido en la Sección~\ref{sec:cap3_cinetica}. La forma funcional de $\tilde{\imath}$ sigue la cinética de Holling~II, cuya parametrización se presenta en la Sección~\ref{sec:cap3_cinetica}.
...
\end{equation}
con $m$ la tasa específica de mantenimiento a la temperatura de referencia (\si{\per\minute}), $B(t)$ la biomasa estructural total del reactor (\si{\gram C}), y $f(T)$ el factor de modulación térmica.
...
\end{equation}
donde $\dot{D}_{\mathrm{larval}}$ es la disipación metabólica directa de las larvas. Los términos $(1-Y_B)\dot{G}_B$ y $(1-Y_L)\dot{G}_L$ capturan el costo de síntesis (\emph{overhead}): la fracción del flujo bruto desde la reserva que no se incorpora a tejido sino que se pierde como \ch{CO2} en las reacciones anabólicas. El término $\dot{D}_{\mu}(t) = \lambda_{\mu} S(t)$ modela la respiración aeróbica de la microbiota asociada al sustrato no ingerido, con $\lambda_{\mu}$ (\si{\per\minute}) la tasa específica de mineralización microbiana del carbono remanente. Esta forma cinética se justifica en la Sección~\ref{sec:cap3_cinetica} como límite de primer orden en sustrato abundante.
...
En conjunto, las ecuaciones~\eqref{eq:cap3_ode_sustrato}--\eqref{eq:cap3_ode_co2} constituyen un sistema de cuatro EDO acopladas no lineales para $\{S, B, L, C_{\ch{CO2}}\}$, con $E$ determinada algebraicamente por el QSS~\eqref{eq:cap3_balance_asimilados}. El sistema se gobierna por los parámetros semilla $\{\kappa_A, m, a_{\max}, Y_B, Y_L\}$ tomados de la literatura \parencite{eriksenDynamicModellingFeed2022,padmanabhaComprehensiveDynamicGrowth2020}, y deja como variables de calibración local en el Capítulo~\ref{chap:hibrido} el subconjunto reducido $\{\rho_{\mathrm{conv}}, \beta_0\}$, correspondientes al factor de conversión biomasa--carbono y al parámetro de escala de la cinética de ingesta. La Sección~\ref{sec:cap3_cinetica} detalla las formas funcionales que cierran los flujos metabólicos.