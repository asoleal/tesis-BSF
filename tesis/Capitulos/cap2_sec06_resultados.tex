...
La validación del módulo de adquisición se fundamenta en tres ejes de desempeño metrológico: (i) linealidad y exactitud frente a patrones certificados, (ii) estabilidad temporal bajo condiciones de operación continuada, y (iii) capacidad de replicabilidad entre unidades. A continuación se presentan los resultados obtenidos durante la caracterización del sensor NDIR MH-Z19B para \ce{CO2} y del MQ-4 para \ce{CH4}, integrados en el nodo IoT descrito en la \cref{sec:cap2_sensores} y sometidos al protocolo de calibración detallado en la \cref{sec:cap2_calibracion}. Los indicadores de desempeño se reportan en términos del coeficiente de correlación de Pearson ($R$), el error porcentual absoluto medio (MAPE) y la incertidumbre expandida ($U$, $k=2$), conforme a los criterios establecidos en la hipótesis H1.

% ---------------------------------------------------------
\textbf{Linealidad y exactitud del sensor NDIR (\ce{CO2}).} La curva de calibración ajustada según la \cref{eq:calibracion_lineal} exhibió una relación lineal prácticamente perfecta en el rango \num{0}--\num{5000}~ppm, con coeficiente de determinación $R^2 = 0{,}9987$ y pendiente $a = 0{,}982 \pm 0{,}012$ como se puede ver en la figura (\cref{fig:cap2_curva_calibracion_co2}). La comparación directa entre las lecturas compensadas del sensor NDIR ($n = 150$ observaciones agrupadas por nivel de patrón) y las concentraciones certificadas arrojó una correlación de Pearson $R = 0{,}997$ ($p <$ \num{0.001}), superando holgadamente el umbral de $R > 0{,}85$ especificado en H1. El MAPE calculado sobre el conjunto completo de validación fue del \SI{9.8}{\percent}, valor inferior al límite del \SI{15}{\percent} y comparable con resultados reportados para sensores NDIR de bajo costo en ambientes controlados de invernadero \parencite{biagiDevelopmentMachineLearningbased2024, yavariArtEMonArtificialIntelligence2023}.
...
\centering
\small
\caption{Métricas de desempeño del sensor NDIR MH-Z19B para \ce{CO2} tras calibración y compensación cruzada ($n = 150$, temperatura \SI{30}{\celsius} $\pm$\,\SI{2}{\celsius}, HR \SI{60}{\percent}--\SI{80}{\percent}).}
...
MAPE (\si{\percent}) & $9{,}8$ & $< 15$ \\
Incertidumbre expandida $U$ ($k=2$) & $\pm$\,\SI{4.9}{\percent} & $< \pm$\,\SI{15}{\percent} \\
